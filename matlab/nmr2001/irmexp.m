%IRMEXP		Fit and plot of relaxation data to a many-exponential recovery 
...
%Usage:
%
%>> [BESTPARS, D_BESTPARS, CHISQUARE, CORR_MATRIX] = IRMEXP(...)
%
%INPUT ARGUMENTS
...
%>>dy=.02*ones(size(x));
%
%>> [a b c] = irmexp(x,y,dy,2);
%or
%>> [a b c] = irmexp([x;y;dy],2);
%
%or (an initial guess is specified)
%
%>> [a b c] = irmexp(x,y,dy,[4 55 3 4 35 2]);
%or
%>> [a b c] = irmexp([x;y;dy],[4 55 3 4 35 2]);
%
%   NMR2001: irmexp 
...
function [varargout]=irmexp(varargin);
...
rdescr=struct('fcn','irfitm','np',2,'offs',1,'interl',1,'pnam',...
	      ['T1 ';'Amp'],'pty', 'ta');
